 ...
 * 下面是存储配置专用的二叉树实现.
 */
CONFIG_BTREE_NODE *config_btree_create_node(const char *key, int key_len,
...
int
config_btree_insert_node(CONFIG_BTREE * config, const char *key,
...
int config_btree_insert_section(CONFIG_BTREE * config,
        const char *section_name)
{
    CONFIG_BTREE *section = config_btree_create();
    if (section == NULL)
        return 0;
    if (config_btree_insert_node
            (config, section_name, 0, &section, sizeof(void *))) {
        return 1;
    } else {
        free(section);
        return 0;
...
CONFIG_BTREE *config_btree_get_section(CONFIG_BTREE * config,
        const char *section_name)
{
...
int config_btree_insert_section_node(CONFIG_BTREE * config,
        const char *section_name,
...
CONFIG_BTREE_NODE *config_btree_find_node(CONFIG_BTREE * config,
        const char *key)
{
    CONFIG_BTREE_NODE *p;
    int comp;
...
int config_btree_inorder_traverse(CONFIG_BTREE_NODE * node,
        CONFIG_BTREE_TRAVERSE_CB callback)
{
    if (node == NULL)
        return 1;
...
int config_btree_inorder_save_traverse(CONFIG_BTREE_NODE * node, FILE * fp,
        CONFIG_BTREE_SAVE_TRAVERSE_CB
        callback)
{
    if (node == NULL)
        return 1;
...
int config_btree_preorder_traverse(CONFIG_BTREE_NODE * node,
        CONFIG_BTREE_TRAVERSE_CB callback)
{
    if (node == NULL)
        return 1;
...
int config_btree_postorder_traverse(CONFIG_BTREE_NODE * node,
        CONFIG_BTREE_TRAVERSE_CB callback)
{
    if (node == NULL)
        return 1;
...
int config_btree_destroy(CONFIG_BTREE * config)
{
    if (!config)
        return 0;
    config_btree_postorder_traverse(config->root,
            config_btree_destroy_section);
    free(config);
    return 1;
}
...
int parser_on_new_line(PARSER * parser)
{
    ...
    switch (parser->status) {
        ...
        case PARSE_STATUS_GET_VALUE:
            k = mem_trim(parser->str + parser->start1,
                    parser->end1 - parser->start1, &klen);
            v = mem_trim(parser->str + parser->start2,
                    parser->pos - parser->start2, &vlen);
            if (k == NULL)
                return 0;
            if (v == NULL) {
                v = ""; /** BUG? */
            ...
            if (!config_btree_insert_section_node
                    (parser->config, parser->section_name, k, klen, v, vlen))
                return 0;
            break;
...
INI_CONFIG *ini_config_create_from_string(char *str, int slen, int isGBK)
{
    ...
    while (parser.pos < slen) {
        ...
        r = funcs[(unsigned char)str[parser.pos]](&parser);
        if (!r) {        // 解析错误,本代码不做任何提示,直接返回 NULL,但可以从 parser 里在这里从parser里取得当前解析的文件位置和当前状态.
            config_btree_destroy(config);
            return NULL;
        }
    }
...
int ini_config_get_int(INI_CONFIG * config, const char *section,
        const char *key, int default_int)
{
    CONFIG_BTREE_NODE *node;
    INI_CONFIG *sec;
    if (section == NULL)
...
char *ini_config_get_string(INI_CONFIG * config, const char *section,
        const char *key, char *default_string)
{
    CONFIG_BTREE_NODE *node;
    INI_CONFIG *sec;
...
int ini_config_set_int(INI_CONFIG * config, const char *section,
        const char *key, int key_len, int value)
{
    char number[32];
    int len = sprintf(number, "%d", value);
    return ini_config_set_string(config, section, key, key_len, number,
            len);
}
...
int ini_config_save_traverse_section(FILE * fp, CONFIG_BTREE_NODE * node)
{
    ...
    memcpy(&section, node->data, sizeof(void *));
    fprintf(fp, "[%s]\n", node->key);
    config_btree_inorder_save_traverse(section->root, fp,
            ini_config_save_traverse_value);
    return 1;
}
...
int ini_config_save(INI_CONFIG * config, const char *filename)
{
    ...
    if (fp == NULL)
        return 0;
    config_btree_inorder_save_traverse(config->root, fp,
            ini_config_save_traverse_section);
    fclose(fp);
    return 1;
}
